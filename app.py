...
def add_product_line(f, cur_section, prod, qty, length, tag):
    ...
    in_posts_section = cur_section == 'Posts'
    if is_rim(prod):
        s = get_sku(prod)
        if in_blocking:
    ...
    elif in_posts_section and is_beam(prod):
        # This VERSA-LAM grade is normally treated as a beam, but the material list is
        # explicitly listing it under "Posts" here - trust the document's own section
        # header for this line rather than the generic grade-based classification.
        ...
        f['posts'].append({'product':norm(prod),'sku':get_sku(prod),'tag':tag,'qty':qty,'length':length})
    elif is_beam(prod) and tag and tag.upper().startswith('LG'):
        # Ledger beams (tag starts with LG) get pulled out of the normal Beams list into
        # their own section, aggregated to a total footage per SKU (no per-line/per-tag rows).
        s = get_sku(prod)
        key = s or norm(prod)
...
def build_cutlist_pdf(cutlist, job_address):
    ...
    for floor_name, f in (cutlist.get('floors') or {}).items():
        ...
        if f.get('rimFt', 0) > 0:
            ...
            pcs = _ceil16(f['rimFt'])
            draw_row([f.get('rimSku') or 'Special order', f.get('rimProd', ''), str(pcs) + ' EA',
                      "16' pcs (" + str(f['rimFt']) + "' / 16)"], [70, 260, 50, 140])
            state['y'] += 6
