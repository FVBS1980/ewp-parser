...
def send_gmail(to_email, subject, html_body, from_name='FVBS EWP Hub'):
    ...
    msg['to'] = to_email
    msg['from'] = f'{from_name} <fvbsewphub@gmail.com>'
    msg['subject'] = subject
    # Gmail expects RFC5322 CRLF line endings for proper DKIM signing/authentication.
    # Python's email lib defaults to bare \n, which can cause messages to be sent
    # unsigned/unauthenticated. Normalize to CRLF before encoding.
...
def send_email_endpoint():
    ...
    try:
        ...
        gresp = send_gmail(to_email, subject, html_body)
        if not gresp.ok:
            return jsonify({'error': f'Gmail send failed: {gresp.status_code}', 'details': gresp.text}), 500
